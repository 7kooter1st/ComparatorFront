...
> Этот черновик **устарел**. Реализована закрытая авторизация: аккаунты создаёт
> админ, анонимных сессий и открытой регистрации нет. Актуальный поток —
> `POST /api/auth/login`, cookie `comparator_session`, история `/api/jobs`.
